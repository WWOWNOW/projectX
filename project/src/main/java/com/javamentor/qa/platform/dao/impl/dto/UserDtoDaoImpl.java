    ...
    public Optional<UserDto> getUserById(Long id) {
        ...
        return entityManager.createQuery(
                        "select new com.javamentor.qa.platform.models.dto.UserDto (" +
                                ...
                                ") " +
                                "from User user where user.id =: id and user.isDeleted = false "
                        , UserDto.class)
                .setParameter("id", id)
                .getResultStream()
                .findAny();
    ...
    public List<UserProfileQuestionDto> getAllDeletedQuestionsByUserId(Long id) {
        return entityManager.createQuery(
                        "select new com.javamentor.qa.platform.models.dto.UserProfileQuestionDto(" +
                                ...
                                "(select (count(ans.id)) from Answer as ans where ans.question.id = q.id), " +
                                "q.persistDateTime)" +
                                "from Question q where q.user.id = :id and q.isDeleted = true " +
                                "order by q.persistDateTime desc, q.id", UserProfileQuestionDto.class)
                .setParameter("id", id)
                .getResultList();
    }
